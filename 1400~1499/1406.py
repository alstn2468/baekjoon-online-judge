...
class Editor:
    def __init__(self, text):
        self.left_stack = deque(text)
        self.right_stack = deque()

    def move_cursor_left(self):
        if self.left_stack:
            self.right_stack.appendleft(self.left_stack.pop())

    def move_cursor_right(self):
        if self.right_stack:
            self.left_stack.append(self.right_stack.popleft())

    def delete_left_char(self):
        if self.left_stack:
            self.left_stack.pop()

    def add_char_to_left(self, char):
